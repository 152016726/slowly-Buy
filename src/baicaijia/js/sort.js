...
function swpie() {
    ...
    // 获取对象
    var menu_ul = document.querySelector('.category_nav');
    ...
    var maxDis = -(menu_ul.offsetWidth - (document.body.offsetWidth - 15));
    // console.log(maxDis);
    ...
    menu_ul.addEventListener("touchmove", function(e) {
        // console.log('move');
        var tmpX = e.targetTouches[0].clientX;

        // 算出 此时滑动了的距离
        var dis = tmpX - startX + distance;
        if (dis > spring) {
            dis = spring;
        ...
        moveDis = dis;
        // 进行位移
        this.style.transform = "translateX(" + dis + "px)";
        // console.log(dis);
    });
    ...
    menu_ul.addEventListener("touchend", function() {
        ...
        this.style.transform = "translateX(" + moveDis + "px)";
    });
    ...
    itcastEvent(menu_ul).tap(function(e) {
        ...
        var lis = document.querySelectorAll(".category_nav>li");
